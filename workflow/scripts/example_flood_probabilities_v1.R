...
# LJS 22 Nov 2020
# Draft code - still need to check if the probabilities section is computed correctly
# https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2020GL091824


rm(list=ls(all=TRUE)); cat('\014')
library(ggplot2);library(gamlss);library(gamlss.dist);library(tidyverse);library(gamlss.foreach)


# Import data
df <- read_csv(paste0("./27009.csv"))
df <- subset(df, select=c(Year,Q))
ggplot(df)+geom_point(aes(x=Year,y=Q))

# Fit model
# http://www.gamlss.com/wp-content/uploads/2013/01/gamlss-manual.pdf
gamlss_model <- gamlss(df$Q ~ df$Year, 
            sigma.formula= ~ df$Year, 
            family="GA", trace = FALSE)

#  Extract parameters
df$mu <- fitted(gamlss_model, what = c("mu"))
df$sigma <- fitted(gamlss_model, what = c("sigma"))

# Extract parameters - official method
mu <- fitted(gamlss_model,what='mu')
sigma <- fitted(gamlss_model,what='sigma')
Q01 <-  qGA(0.01,mu,sigma)
Q95 <- qGA(0.95,mu,sigma)
Q98 <- qGA(0.98,mu,sigma)
Q99 <- qGA(0.99,mu,sigma)
a <- data.frame("Year"= df$Year,
                  "Q01"=Q01,
                  "Q95"=Q95,
                  "Q98"=Q98,
                  "Q99"=Q99,
...
# Extract the magnitude of the 20-y, 50-y and 100-y flow
v20_1950 = round(a[a$Year==1950,"Q95"],6) 
v20_1970 = round(a[a$Year==1970,"Q95"],6) 
v20_2015 = round(a[a$Year==2015,"Q95"],6)
